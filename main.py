...
def isinrow (board, gridsize, row, trynum):
    for i in range(gridsize):
        if board[row][i] == trynum:
            return True
    return False

def isincol(board, gridsize, col, trynum):
    for i in range(gridsize):
        if board[i][col] == trynum:
            return True
    return False

def isinbox(board, gridsize, row, col, trynum):
...
gridsize = gridsize ** 2
print()
board = [[0] * gridsize for i in range(gridsize)]
...
# Display solve (board or unsolveable)